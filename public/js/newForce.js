//get the payload that will be sent to server
var payload = document.getElementById("script").getAttribute("jsonObj");

if (payload) {
	//make request and send payload to server
	var req = new XMLHttpRequest();
	req.open('POST', 'https://webcrawler-201200.appspot.com', true);
	req.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	req.onload = createGraph;
	//If an error occurs while communicating with server, then reveal and display an error message.
	req.onerror = function (e) {
		var header = document.createElement('h2');
		var body = document.createElement('h3');
		header.innerText = 'Error Communicating with Server';
		body.innerText = 'The client encountered an error while trying to communicate with the server. Please try to again in 30 seconds. Additionally, it is possible that the page provided generates an error from the server, in which case a different URL should be used.';
		document.getElementById("error").appendChild(header);
		document.getElementById("error").appendChild(body);
		//reveal error div
...
//function reveals the graph div and hides the loading icon
function revealGraph() {
	document.getElementById("graph").style.display = "flex";
	document.getElementById("loader").style.display = "none";
}

// Function uses the response from the server in order to populate a graph with nodes and edges to represent the sites the crawler visited.
// Function also implements all manipulations for the graph.
function createGraph() {
	//if server returned a status code associated with an error, then display the error response and don't populate graph.
	if (req.status >= 400) {
